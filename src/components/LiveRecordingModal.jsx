...
export default function LiveRecordingModal({ onStop = () => { }, onCancel = () => { } }) {
  ...
  const [transcriptionFailed, setTranscriptionFailed] = useState(false);   // ← add here
    ...
    if (SpeechRecognition) {
      try {
        ...
        recognition.onresult = (event) => {
          ...
          // Loop only from resultIndex to avoid reprocessing old results
          ...
          // Only append to permanent transcript when final
          if (finalText) {
            const separator = transcriptRef.current && !transcriptRef.current.endsWith(' ') ? ' ' : '';
            transcriptRef.current = transcriptRef.current + separator + finalText.trim();
          }

          // Live pill shows final-so-far + whatever's currently interim
          const displayText = interimText ? `${transcriptRef.current} ${interimText}`.trim() : transcriptRef.current;
  ...
  const handleStopRecording = () => {
    ...
    console.log(samplesRef.current)
    setTimeout(() => {
      onStop(payload);
    }, 120);
  };
    ...
    <div className={`live-recording-modal ${isExiting ? 'live-recording-modal--exiting' : ''}`}>
      ...
      <div className="listening-status-wrapper">
        <div className="listening-status-pill">
          {transcriptionFailed ? 'Voice terrain still forming — live captions unavailable right now' : liveTranscript ? `"${liveTranscript.slice(-38)}..."`
            : liveBiomeHint}
        </div>
      </div>
